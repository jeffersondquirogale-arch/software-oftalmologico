...
export const LensometriaTab = ({ historia, setHistoria }: LensometriaTabProps) => {
  ...
  const odF = ['lensOD_add', 'lensOD_dnp', 'lensOD_pris'] as const;
  const oiF = ['lensOI_add', 'lensOI_dnp', 'lensOI_pris'] as const;

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      <div>
        <p style={S.title}>Lensometría</p>
        <p style={S.subtitle}>Medición de los lentes actuales</p>
      </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
          ...
          <tbody>
            <tr>
              ...
              {odF.map(f => (
                <td key={f} style={S.td}>
                  <input type="text" value={historia[f] || ''} onChange={set(f)}
                    style={S.inputSm} onFocus={S.onFocus} onBlur={S.onBlur} />
                </td>
              ))}
            </tr>
            <tr>
